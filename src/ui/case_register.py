    ...
    def delete_selected_entries(self, table):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            for index in selection:
                ...
                # Try to find CaseID or ID
                case_id = case_data.get('CaseID') or case_data.get('id')
                
                res = False
                if case_id:
                    # Try deleting as CSV case first (most register entries are from CSV/Case Files)
                    res = self.db.delete_csv_case(case_id)
                    # If failed, try SQLite (though registers mostly pull from CSV/Case Files currently)
                    if not res:
